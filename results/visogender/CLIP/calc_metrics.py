...
for method in ["clip", "vision_frozen", "text_frozen", "none_frozen", "projections_not_frozen"]:
    # Load JSON files
    print(f"-------------- Method: {method} --------------")
    print()
    model_type = method
    with open(f"{model_type}/clip_{model_type}_ContextOO.json", "r") as f:
        oo_data = json.load(f)

    with open(f"{model_type}/clip_{model_type}_ContextOP.json", "r") as f:
        op_data = json.load(f)
